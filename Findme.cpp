...
int main()
{
	std::cout << "Loading Math" << std::endl;
	Sleep(2000);
	std::string GameWindow = "Overwatch";
...
bool CompareColour(RGBQUAD *pPixels, int height, int width, int x, int y)
{
	...
	int p = (height - y - 1)*width + x; // upside down
										...
										//PINK
										//CSS BLUE
	if (((int)pPixels[p].rgbRed > 245 && (int)pPixels[p].rgbGreen < 20 && (int)pPixels[p].rgbBlue < 20 ) && ((int)pPixels[p].rgbBlue > 0) && ((int)pPixels[p].rgbGreen == 0))
	{

		//std::cout << (int)pPixels[p + 20].rgbRed << ", " << (int)pPixels[p + 20].rgbBlue << ", " << (int)pPixels[p+20].rgbGreen << std::endl;
...
void ScanBMPHorizontal(ScanContents * scan)
{
		...
		//for(scan->Y = scan->StartingY-((int)(scan->RcWindow.bottom - scan->RcWindow.top)/5)/*GET'S THE WINDOW'S HEIGHT/5*/; scan->Y < scan->StartingY + ((int)(scan->RcWindow.bottom - scan->RcWindow.top)/5); scan->Y+=3)
	{
		...
		for (int x = (int)((scan->RcWindow.right - scan->RcWindow.left) / 5)/*+15*/;
			x < (int)((scan->RcWindow.right - scan->RcWindow.left) - (scan->RcWindow.right - scan->RcWindow.left) / 5);
			x++)
		{
			...
			if (CompareColour(scan->PPixels, scan->Bm.bmHeight, scan->Bm.bmWidth, x, y))
			{
				...
				int calcX = (((z - (z - x) / 2) + scan->RcWindow.left) + 40);
				int calcY = (((y + 4) + scan->RcWindow.top) + 60);
				if (oldCoordX != 0)
				{
					leaddeltaX = calcX - oldCoordX;
					leaddeltaY = calcY - oldCoordY;
				}
				if (leaddeltaX > -3 & leaddeltaX < 3) {
					leaddeltaX = 0;
				}
				if (leaddeltaY > -5 & leaddeltaY < 5) {
					leaddeltaY = 0;
				}
				if (leaddeltaX > 50 || leaddeltaX < -50 || leaddeltaY > 50 || leaddeltaY < -50)
				{
				...
				//std::cout << "z: " << calcY << std::endl;
				//std::cout << "oldx: " << oldCoordX << std::endl;
				//std::cout << "oldy: " << oldCoordY << std::endl;
				if (!((calcX + leaddeltaX) == 800)) {
					calcX++;
					shootthem(calcX, calcY );
				}
				if (!((calcY + leaddeltaX) == 450)) {
					calcY++;
					shootthem(calcX, calcY);
				}
				shootthem(calcX + leaddeltaX, calcY + leaddeltaY);
				oldCoordX = calcX + leaddeltaX;
				oldCoordY = calcY + leaddeltaY;

				//std::cout << "Coord X:" << (z - (z - x) / 2) + scan->RcWindow.left << " Y: " << (y + 4) + scan->RcWindow.top << " Current X:" << CurrentMouseXY.X << " Current Y:" << CurrentMouseXY.Y << std::endl;
				...
				//{
				//	if ((currentPos.x < (((z - (z - x) / 2) + scan->RcWindow.left) + 5)) && (currentPos.x > (((z - (z - x) / 2) + scan->RcWindow.left) - 5)) && (currentPos.y < ((y + scan->RcWindow.top) + 5 )) && (currentPos.y > ((y + scan->RcWindow.top) - 5))) {
...
bool Aim_BotThr(AimbotThr * aimThr1)
{
	...
	while (true)
	{
		...
		if (!GetAsyncKeyState(20)) //VK_RBUTTON
		{
